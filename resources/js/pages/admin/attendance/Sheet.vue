...
type Props = {
    ...
    section?: { id: number; subject: string; cohort: string; scheduleDisplay: string }
}
...
setLayoutProps({
    breadcrumbs: [
        { title: 'Admin', href: '#' },
        { title: 'Asistencia', href: adminSectionIndex.url({ section: props.sheet.session.sectionId }) },
        { title: props.sheet.session.topic || 'Sesión', href: '#' },
    ],
})

// ---- State ----------------------------------------------------------------

/** Initialize every student as 'present' — start present, mark absent */
...
const sectionId = props.sheet.session.sectionId
const sessionId = props.sheet.session.id
const { save, processing, saved } = useAdminAttendanceSheetForm(sectionId, sessionId)
...
function goBack(): void {
    router.visit(adminSectionIndex.url({ section: sectionId }))
}

function handleSave(): void {
    // Admin save always uses professor_present=false (forced server-side)
    save(marks.value)
}
    ...
    <div class="att-rc" role="dialog" aria-modal="true">
        ...
        <div class="att-rc-body">
            <div class="att-rc-inner">
                <!-- Admin banner (always shown) -->
                <div class="att-admin-banner" style="margin-bottom: 18px;">
